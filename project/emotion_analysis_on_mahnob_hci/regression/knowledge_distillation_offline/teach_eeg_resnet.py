...
from project.emotion_analysis_on_mahnob_hci.regression.knowledge_distillation_offline.trainer import \
    MahnobResnetKnowledgeDistillationTrainer
...
class TeacherEEGResNet(GenericExperiment):
    ...
    def create_model(self):
        ...
        student_backbone_state_folder = self.config['kd_config']['resnet']['student_eeg_image_backbone_state_folder']

        teacher = kd_res50(num_classes=8, state_dict_name=self.backbone_state_dict_frame, mode=self.backbone_mode,
                           embedding_dim=512, input_channels=3,
                           root_dir=self.model_load_path, folder='', role="teacher")

        student = kd_res50(num_classes=8, state_dict_name=self.backbone_state_dict_eeg, mode=self.backbone_mode,
                           embedding_dim=512, input_channels=3,
    ...
    def experiment(self):
        ...
        for fold in iter(self.folds_to_run):
            ...
            trainer = MahnobResnetKnowledgeDistillationTrainer(model, teacher, model_name=self.model_name,
                                                               save_path=fold_save_path,
                                                               criterion=criterion,
                                                               num_classes=8,
                                                               device=self.device, factor=self.factor,
                                                               learning_rate=self.learning_rate, fold=fold,
                                                               milestone=[0],
...
if __name__ == '__main__':
    ...
    experiment_handler = TeacherEEGResNet(args)
