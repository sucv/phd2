...
class knowledge_extraction(GenericExperiment):
    ...
    def load_config(self):
        ...
        from project.emotion_analysis_on_mahnob_hci.configs import config_mahnob
        from project.emotion_analysis_on_mahnob_hci.regression.knowledge_distillation_offline.configs import config_knowledge_distillation
        config = {'generic': config_mahnob, 'kd': config_knowledge_distillation}
    ...
    def create_model(self):
        ...
        if "2d1d" in self.model_name:
            model = kd_2d1d(backbone_state_dict=backbone_state_dict, backbone_mode=self.backbone_mode,
    ...
    def init_dataloader(self, subject_id_of_all_folds, fold_arranger, fold, class_labels=None):
        ...
        for partition in partition_dictionary.keys():
            dataset = MAHNOBDataset(self.config['generic'], data_dict[partition], normalize_dict=normalize_dict, modality=self.modality,
                                    continuous_label_frequency=self.config['generic']['frequency_dict']['continuous_label'], normalize_eeg_raw=self.normalize_eeg_raw,
